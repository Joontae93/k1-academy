 ...
 */
function k1academy_enqueue_page_style( string $id, array $deps = array( 'global' ) ) {
 ...
 * @param array  $deps Optional array of dependencies.
 */
function k1academy_enqueue_page_script( string $id, array $deps = array( 'global' ) ) {
	$asset_file = get_stylesheet_directory() . "/dist/pages/{$id}.asset.php";
 ...
 */
function k1academy_enqueue_page_assets( string $id, array $deps = array() ) {
	$default_deps = array(
		'styles'  => array( 'global' ),
	...
	k1academy_enqueue_page_style( $id, $deps['styles'] );
	k1academy_enqueue_page_script( $id, $deps['scripts'] );
}
